#!/usr/bin/env python3
"""Frozen R7 directional path-energy asymmetry Stage-1 screen.

Runs one preregistered directional-energy score at frozen S1/S2 causal
completed-wave confirmations. Reads CSI1000 only through 2022-12-31; the
2023-2025 reserve and all 2026 rows remain unopened.
"""
from __future__ import annotations

import argparse
import hashlib
import json
import subprocess
import sys
from pathlib import Path

import numpy as np
import pandas as pd
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import brier_score_loss, log_loss
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "scripts"))

import run_rmr_stage1_common_probe as common
import run_rmr_R5_stage1 as r5

PROTOCOL = ROOT / "docs/governance/reversal_mean_reversion_R7_stage1_protocol_v1.json"
PREANALYSIS = ROOT / "docs/research/rmr_R7_directional_path_energy_asymmetry_preanalysis_20260908.md"
DATA_ROLES = ROOT / "docs/governance/reversal_mean_reversion_stage1_common_data_roles_v1.json"
SCALE_CONTRACT = ROOT / "docs/governance/reversal_mean_reversion_stage1_scale_contract_v1.json"
DEFAULT_SOURCE = ROOT / "data/high_open_dev_2015_2025/1m_official.parquet"
OUT = ROOT / "docs/research/local_rmr_R7_stage1_receipt_v1.json"
USAGE = ROOT / "docs/governance/local_rmr_R7_stage1_data_usage_v1.json"

EXPECTED_SHA = "11f4a5e78381371680fbcf6e01891216f645a8de869646ced6a623970727bcce"
SYMBOL = "000852.SH"
DEV_END = "2019-12-31"
STAB_START = "2020-01-01"
STAB_END = "2022-12-31"
WINDOW = 240
MIN_VALID_RETURNS = 120
REFERENCE_WINDOW = 100
HORIZON_BARS = 1200
SCALES = ("S1", "S2")
SCORE_ID = "R7_directional_energy_balance"


def load_json(path: Path) -> dict:
    return json.loads(path.read_text(encoding="utf-8"))


def dump_json(path: Path, payload: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def sha256(path: Path) -> str:
    h = hashlib.sha256()
    with path.open("rb") as handle:
        for block in iter(lambda: handle.read(1024 * 1024), b""):
            h.update(block)
    return h.hexdigest()


def git_head() -> str | None:
    try:
        return subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True, stderr=subprocess.DEVNULL
        ).strip()
    except Exception:
        return None


def validate_contracts() -> tuple[dict, dict, dict]:
    p = load_json(PROTOCOL)
    roles = load_json(DATA_ROLES)
    scales = load_json(SCALE_CONTRACT)
    if p["research_identity"] != "R7_directional_path_energy_asymmetry_stage1_v1":
        raise RuntimeError("R7 identity drifted")
    if p["stage"] != "results_blind_R7_stage1_protocol_freeze_before_empirical_outcome_open":
        raise RuntimeError("R7 stage drifted")
    if p["market_outcomes_opened_for_protocol_design"] is not False:
        raise RuntimeError("R7 protocol was not results blind")
    if p["state_measurement"]["score_id"] != SCORE_ID:
        raise RuntimeError("R7 score id drifted")
    if int(p["state_measurement"]["trailing_observed_1m_bars"]) != WINDOW:
        raise RuntimeError("R7 state window drifted")
    if int(p["state_measurement"]["minimum_valid_within_session_returns"]) != MIN_VALID_RETURNS:
        raise RuntimeError("R7 minimum valid-return count drifted")
    if int(p["normalization"]["reference_window"]) != REFERENCE_WINDOW:
        raise RuntimeError("R7 normalization window drifted")
    if tuple(p["event_scales"]["reported_scales"]) != SCALES:
        raise RuntimeError("R7 event scales drifted")
    if roles["source"]["sha256"] != EXPECTED_SHA:
        raise RuntimeError("R7 source identity drifted")
    if roles["broad_program_roles"]["STAGE1_RESERVE"]["open_now"] is not False:
        raise RuntimeError("R7 reserve unexpectedly open")
    if scales["source"] != roles["source"]["path"]:
        raise RuntimeError("R7 scale/source mismatch")
    return p, roles, scales


def read_source(path: Path) -> tuple[pd.DataFrame, dict]:
    actual = sha256(path)
    if actual != EXPECTED_SHA:
        raise RuntimeError(f"R7 source SHA mismatch: {actual}")
    frame = pd.read_parquet(
        path,
        columns=["symbol", "trading_day", "timestamp", "close"],
        filters=[("trading_day", "<=", STAB_END)],
    )
    frame["symbol"] = frame["symbol"].astype(str)
    frame["trading_day"] = frame["trading_day"].astype(str)
    frame["close"] = pd.to_numeric(frame["close"], errors="coerce")
    frame = frame.loc[frame["symbol"].eq(SYMBOL)].copy()
    if frame.empty or str(frame["trading_day"].max()) > STAB_END:
        raise RuntimeError("R7 source boundary invalid")
    frame = frame.sort_values(["trading_day", "timestamp"], kind="mergesort").reset_index(drop=True)
    close = frame["close"].to_numpy(dtype=float)
    if not np.isfinite(close).all() or (close <= 0).any():
        raise RuntimeError("R7 invalid close values")
    frame["_idx"] = np.arange(len(frame), dtype=int)
    return frame, {
        "source_sha256": actual,
        "rows_loaded": int(len(frame)),
        "min_day": str(frame["trading_day"].min()),
        "max_day": str(frame["trading_day"].max()),
        "rows_2023_or_later": 0,
        "reserve_2023_2025_opened": False,
    }


def frozen_thresholds(frame: pd.DataFrame) -> tuple[float, dict[str, float]]:
    temp = frame.copy()
    temp["clock"] = temp["timestamp"].astype(str).str[11:16]
    daily = temp.loc[temp["clock"].eq("15:00"), ["trading_day", "close"]].drop_duplicates(
        "trading_day", keep="last"
    ).sort_values("trading_day")
    daily["ret"] = daily["close"].pct_change(fill_method=None)
    daily["rv20"] = daily["ret"].rolling(20, min_periods=20).std().shift(1)
    vol_ref = float(daily.loc[daily["trading_day"] <= DEV_END, "rv20"].dropna().median())
    if not np.isfinite(vol_ref) or vol_ref <= 0:
        raise RuntimeError("R7 invalid DEV median rvol20")
    return vol_ref, {"S1": 0.25 * vol_ref, "S2": 0.50 * vol_ref}


def trailing_within_session_returns(
    log_prices: np.ndarray,
    days: np.ndarray,
    end_idx: int,
    window: int = WINDOW,
) -> np.ndarray:
    start = max(1, int(end_idx) - int(window) + 1)
    vals: list[float] = []
    for j in range(start, int(end_idx) + 1):
        if str(days[j]) != str(days[j - 1]):
            continue
        value = float(log_prices[j] - log_prices[j - 1])
        if np.isfinite(value):
            vals.append(value)
    return np.asarray(vals, dtype=float)


def directional_energy_balance(
    log_prices: np.ndarray,
    days: np.ndarray,
    end_idx: int,
    wave_direction: int,
) -> float:
    if int(wave_direction) not in (-1, 1):
        return float("nan")
    returns = trailing_within_session_returns(log_prices, days, end_idx)
    if len(returns) < MIN_VALID_RETURNS:
        return float("nan")
    same = returns * int(wave_direction) > 0.0
    opp = returns * int(wave_direction) < 0.0
    same_energy = float(np.sum(np.square(returns[same])))
    opp_energy = float(np.sum(np.square(returns[opp])))
    total = same_energy + opp_energy
    if not np.isfinite(total) or total <= 0:
        return float("nan")
    score = (same_energy - opp_energy) / total
    return float(np.clip(score, -1.0, 1.0))


def build_score_table(
    frame: pd.DataFrame,
    prices: np.ndarray,
    waves: list[common.Wave],
    threshold: float,
) -> pd.DataFrame:
    log_prices = np.log(prices)
    days = frame["trading_day"].to_numpy(dtype=str)
    rows: list[dict] = []
    for wave in waves:
        idx = int(wave.confirm_idx)
        if idx >= len(frame):
            continue
        day = str(days[idx])
        if day > STAB_END:
            continue
        value = directional_energy_balance(log_prices, days, idx, int(wave.direction))
        rows.append({
            "property_id": SCORE_ID,
            "day": day,
            "confirm_idx": idx,
            "wave_direction": int(wave.direction),
            "severity": abs(float(wave.move)) / float(threshold),
            "property_value": float(value) if np.isfinite(value) else np.nan,
        })
    table = pd.DataFrame(rows).sort_values("confirm_idx", kind="mergesort").reset_index(drop=True)
    if table.empty:
        return table
    table["z"] = r5.rolling_median_mad_z(table["property_value"].tolist(), REFERENCE_WINDOW)
    table["next_z"] = table["z"].shift(-1)
    return table


def fit_model(train: pd.DataFrame, features: list[str]) -> Pipeline | None:
    clean = train.dropna(subset=features + ["y"]).copy()
    if len(clean) < 30 or clean["y"].nunique() < 2:
        return None
    pipe = Pipeline([
        ("sc", StandardScaler()),
        ("lr", LogisticRegression(
            C=1.0, penalty="l2", solver="lbfgs", fit_intercept=True,
            class_weight=None, max_iter=1000,
        )),
    ])
    pipe.fit(clean[features].to_numpy(dtype=float), clean["y"].to_numpy(dtype=int))
    return pipe


def score_model(model: Pipeline | None, data: pd.DataFrame, features: list[str]) -> dict:
    clean = data.dropna(subset=features + ["y"]).copy()
    if model is None or clean.empty:
        return {"status": "evidence_insufficient", "n": int(len(clean))}
    y = clean["y"].to_numpy(dtype=int)
    p = model.predict_proba(clean[features].to_numpy(dtype=float))[:, 1]
    return {
        "status": "scored",
        "n": int(len(clean)),
        "event_rate": float(np.mean(y)),
        "brier": float(brier_score_loss(y, p)),
        "log_loss": float(log_loss(y, p, labels=[0, 1])),
        "mean_probability": float(np.mean(p)),
    }


def summarize_scale(events: pd.DataFrame, raw: pd.DataFrame) -> dict:
    resolved = events.loc[events["outcome"].isin(["reversion", "extension"])].copy()
    if not resolved.empty:
        resolved["y"] = resolved["outcome"].eq("reversion").astype(int)
    dev = resolved.loc[resolved["day"] <= DEV_END].copy()
    stability = resolved.loc[
        (resolved["day"] >= STAB_START) & (resolved["day"] <= STAB_END)
    ].copy()
    base_f = ["severity"]
    aug_f = ["severity", "z"]
    base = fit_model(dev, base_f)
    aug = fit_model(dev, aug_f)
    pooled = {
        "baseline": score_model(base, stability, base_f),
        "augmented": score_model(aug, stability, aug_f),
    }
    if pooled["baseline"].get("status") == "scored" and pooled["augmented"].get("status") == "scored":
        pooled["augmented_minus_baseline_brier"] = float(
            pooled["augmented"]["brier"] - pooled["baseline"]["brier"]
        )
        pooled["augmented_minus_baseline_log_loss"] = float(
            pooled["augmented"]["log_loss"] - pooled["baseline"]["log_loss"]
        )
    annual = {}
    counts = {}
    improved = 0
    for year in (2020, 2021, 2022):
        part = stability.loc[stability["day"].str.startswith(str(year))].copy()
        b = score_model(base, part, base_f)
        a = score_model(aug, part, aug_f)
        annual[str(year)] = {"baseline": b, "augmented": a}
        counts[str(year)] = int(len(part))
        if b.get("status") == "scored" and a.get("status") == "scored" and a["brier"] < b["brier"]:
            improved += 1
    gates = {
        "minimum_resolved_stability_events": int(len(stability)) >= 200,
        "minimum_50_each_stability_year": all(v >= 50 for v in counts.values()),
        "pooled_brier_strictly_lower": bool(
            pooled["baseline"].get("status") == "scored"
            and pooled["augmented"].get("status") == "scored"
            and pooled["augmented"]["brier"] < pooled["baseline"]["brier"]
        ),
        "pooled_logloss_strictly_lower": bool(
            pooled["baseline"].get("status") == "scored"
            and pooled["augmented"].get("status") == "scored"
            and pooled["augmented"]["log_loss"] < pooled["baseline"]["log_loss"]
        ),
        "annual_brier_improvement_at_least_2_of_3": improved >= 2,
    }
    return {
        "inventory": {
            "raw_observations": int(len(raw)),
            "normalized_observations": int(raw["z"].notna().sum()),
            "price_outcome_events": int(len(events)),
            "resolved": int(len(resolved)),
            "censored": int((events["outcome"] == "censored").sum()),
            "dev_resolved": int(len(dev)),
            "stability_resolved": int(len(stability)),
            "stability_by_year": counts,
        },
        "price_path_model_comparison": {"pooled_stability": pooled, "annual": annual},
        "descriptive_score_reversion": r5.property_reversion_stats(
            raw.loc[(raw["day"] >= STAB_START) & (raw["day"] <= STAB_END)]
        ),
        "scale_local_gate": gates,
        "scale_local_gate_passed": bool(all(gates.values())),
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--parquet", type=Path, default=DEFAULT_SOURCE)
    parser.add_argument("--output", type=Path, default=OUT)
    parser.add_argument("--usage-output", type=Path, default=USAGE)
    args = parser.parse_args()

    validate_contracts()
    frame, source_audit = read_source(args.parquet.resolve())
    vol_ref, thresholds = frozen_thresholds(frame)
    prices = frame["close"].to_numpy(dtype=float)
    results = {}
    for scale in SCALES:
        waves = common.detect_waves(prices, thresholds[scale])
        raw = build_score_table(frame, prices, waves, thresholds[scale])
        events = r5.add_price_outcomes(raw, prices, thresholds[scale])
        results[scale] = {
            "threshold": float(thresholds[scale]),
            "completed_wave_count": int(len(waves)),
            "score": summarize_scale(events, raw),
        }

    qualifies = bool(all(results[s]["score"]["scale_local_gate_passed"] for s in SCALES))
    receipt = {
        "schema_id": "factorlab_reversal_mean_reversion_R7_stage1_receipt@1.0",
        "session_date": "2026-09-08",
        "program_identity": "broad_reversal_mean_reversion_discovery_program_v1",
        "research_identity": "R7_directional_path_energy_asymmetry_stage1_v1",
        "stage": "R7_stage1_screen_complete_pending_program_review_if_positive",
        "code_commit": git_head(),
        "protocol": str(PROTOCOL.relative_to(ROOT)),
        "preanalysis": str(PREANALYSIS.relative_to(ROOT)),
        "runner_sha256": sha256(Path(__file__)),
        "source": source_audit,
        "vol_ref_dev_median_rvol20": float(vol_ref),
        "directional_change_thresholds": {k: float(v) for k, v in thresholds.items()},
        "state_window_observed_bars": WINDOW,
        "minimum_valid_within_session_returns": MIN_VALID_RETURNS,
        "normalization_reference_window": REFERENCE_WINDOW,
        "score_id_exact": SCORE_ID,
        "scales_exact": list(SCALES),
        "results": results,
        "candidate_for_program_review": qualifies,
        "positive_result_label": "candidate_for_program_review_only" if qualifies else None,
        "third_mechanism_auto_promoted": False,
        "reserve_2023_2025_opened": False,
        "2026_rows_loaded": False,
        "state_window_search_performed": False,
        "alternative_asymmetry_definition_search_performed": False,
        "z_threshold_search_performed": False,
        "scale_search_performed": False,
        "hyperparameter_search_performed": False,
        "interaction_used": False,
        "probability_calibration_performed": False,
        "binary_threshold_selection_performed": False,
        "trading_return_used": False,
        "deep_model_used": False,
        "raw_event_rows_written_to_repo": False,
        "scientifically_fresh": False,
        "production_authority": False,
    }
    dump_json(args.output.resolve(), receipt)
    usage = {
        "schema_id": "factorlab_reversal_mean_reversion_R7_stage1_data_usage@1.0",
        "session_date": "2026-09-08",
        "research_identity": "R7_directional_path_energy_asymmetry_stage1_v1",
        "opened": {
            "CSI1000_2015-01-05_to_2019-12-31": "development_material",
            "CSI1000_2020-01-01_to_2022-12-31": "chronological_stability_not_fresh"
        },
        "sealed_or_unopened": {
            "CSI1000_2023-01-01_to_2025-12-31_internal_reserve": True,
            "all_2026_rows": True
        },
        "aggregate_outputs_only": True,
        "raw_rows_written_to_repo": False,
        "trading_return_used": False,
        "scientifically_fresh": False,
        "production_authority": False
    }
    dump_json(args.usage_output.resolve(), usage)
    print("RMR_R7_STAGE1_RESULT", json.dumps({
        "candidate_for_program_review": qualifies,
        "reserve_2023_2025_opened": False,
        "2026_rows_loaded": False,
        "third_mechanism_auto_promoted": False
    }, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
