#!/usr/bin/env python3
"""Reusable low-bandwidth BLACKBOX certifier for R1 economic translation v3."""
...
DEFAULT_RECEIPT = ROOT / "docs/research/local_rmr_R1_economic_translation_v3_blackbox_receipt.json"
...
def validate_freeze(path: Path) -> tuple[dict, dict]:
    ...
    if freeze["research_identity"] != "rmr_R1_parent_integrity_economic_translation_v3":
        raise RuntimeError("economic v3 blackbox identity drifted")
    if freeze["strategy_candidate_id"] != "R1_DIRECT_RETURN_EDGE_POSITIVE_NEXT_BAR_10BP":
        raise RuntimeError("economic v3 candidate drifted")
    if freeze["blackbox_used_in_fit"] is not False:
        raise RuntimeError("economic v3 fit used blackbox")
    if float(freeze["ridge_alpha"]) != 1.0:
        raise RuntimeError("economic v3 ridge alpha drifted")
    if freeze["baseline_features"] != BASE_FEATURES or freeze["candidate_features"] != CAND_FEATURES:
        raise RuntimeError("economic v3 frozen features drifted")
    expected = freeze["parameter_bundle_sha256"]
    check = dict(freeze)
    check.pop("parameter_bundle_sha256")
    if json_digest(check) != expected:
        raise RuntimeError("economic v3 final bundle digest mismatch")
    return protocol, freeze


def read_frame(path: Path, expected_sha: str, start: str, end: str) -> pd.DataFrame:
    if sha256(path) != expected_sha:
        raise RuntimeError("economic v3 source identity mismatch")
    ...
    if frame.empty:
        raise RuntimeError("economic v3 source empty")
...
def combine(hist: pd.DataFrame, blackbox: pd.DataFrame) -> pd.DataFrame:
    if str(hist["trading_day"].max()) != "2025-12-31":
        raise RuntimeError("economic v3 historical context end drifted")
    if str(blackbox["trading_day"].min()) != BLACKBOX_START or str(blackbox["trading_day"].max()) != BLACKBOX_END:
        raise RuntimeError("economic v3 blackbox window incomplete")
    ...
    if out.duplicated(["symbol", "trading_day", "timestamp"]).any():
        raise RuntimeError("economic v3 combined source duplicate minute")
    return out.reset_index(drop=True)


def frozen_return_predict(snapshot: dict, x: np.ndarray) -> np.ndarray:
    ...
    coef = np.asarray(snapshot["ridge"]["coef"], dtype=float)
    intercept = float(snapshot["ridge"]["intercept"])
    return z @ coef + intercept


def add_frozen_predictions(events: pd.DataFrame, frozen_pair: dict) -> pd.DataFrame:
    out = frozen_eval.add_frozen_integrity(events, frozen_pair["parent_feature_scaler"])
    pred_base = frozen_return_predict(
        frozen_pair["baseline_return_model"], out[BASE_FEATURES].to_numpy(float)
    )
    pred_cand = frozen_return_predict(
        frozen_pair["candidate_return_model"], out[CAND_FEATURES].to_numpy(float)
    )
...
def pairing_gate(events: pd.DataFrame, frozen_pair: dict, minimum: int) -> tuple[bool, bool]:
    ...
    scored = add_frozen_predictions(bb, frozen_pair)
...
def main() -> int:
    ...
    query_id = hashlib.sha256(f"R1ECONV3|{candidate_fp}|{protocol_fp}|{BLACKBOX_SHA}".encode()).hexdigest()[:20]
    receipt = {
