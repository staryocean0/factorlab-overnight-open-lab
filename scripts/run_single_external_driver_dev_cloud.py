...
CONFIG = {
    "overnight_china_offshore_driver_v1": {
        "candidate": "china_offshore_z",
        "raw": "a50_channel_return",
        "sign": 1.0,
        ...
        "state": "docs/governance/china_offshore_driver_v1_state.json",
    },
    "overnight_fx_cny_driver_v1": {
        "candidate": "fx_cny_z",
        "raw": "hkma_usdcny_closure_return",
        "sign": -1.0,
        "product_family": "OFP-B3_fx_macro_overnight_driver",
        "output": "docs/research/cloud_fx_cny_driver_v1_dev_diagnostic.json",
        "state": "docs/governance/fx_cny_driver_v1_state.json",
    },
}
...
def diag(frame: pd.DataFrame,candidate:str,min_n:int)->dict:
    ...
    bb,*_=np.linalg.lstsq(db,y,rcond=None); bc,*_=np.linalg.lstsq(dc,y,rcond=None)
    rb,rc=r2(y,db@bb),r2(y,dc@bc)
    cr=residualize(c,xb); yr=residualize(y,xb)
    pc=None if np.std(cr)==0 or np.std(yr)==0 else float(np.corrcoef(cr,yr)[0,1])
    z=x.copy()
    for col in cols:
...
def load_frame()->tuple[pd.DataFrame,dict]:
    ...
    pieces=[]; hashes={}
    for year in YEARS:
        fp=FACTOR_ROOT/f'factor_panel_{year}.csv'; dp=DRIVER_ROOT/f'driver_external_{year}.csv'
        f=pd.read_csv(fp); d=pd.read_csv(dp)
        ...
        x=f.merge(d[['trading_day','a50_channel_return','hkma_usdcny_closure_return']],on='trading_day',how='inner',validate='one_to_one')
        x['year']=year; pieces.append(x)
...
def main()->int:
    ...
    if ledger.get('query_count')!=8:raise RuntimeError('unexpected ledger count before B2/B3 DEV')
    ...
    print('SINGLE_EXTERNAL_DRIVER_DEV_DIAGNOSTIC_COMPLETE'); print(decision); return 0
