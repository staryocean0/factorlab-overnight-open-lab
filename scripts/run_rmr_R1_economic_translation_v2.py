#!/usr/bin/env python3
"""R1 economic translation v2 on reusable DEV / VALIDATION.

V2 keeps the frozen R1 mechanism, next-bar entry, original structural exits and
10bp round-trip cost.  The only new rule is an ex-ante binary-boundary
structural expectancy filter.  This runner reads no 2026 BLACKBOX data.
"""
...
DEFAULT_RECEIPT = ROOT / "docs/research/local_rmr_R1_economic_translation_v2_validation_receipt.json"
...
def validate_protocol() -> dict:
    ...
    if p["research_identity"] != "rmr_R1_parent_integrity_economic_translation_v2":
        raise RuntimeError("economic v2 identity drifted")
    s = p["strategy_candidate"]
    if s["id"] != "R1_STRUCTURAL_EXPECTANCY_POSITIVE_NEXT_BAR_10BP":
        raise RuntimeError("economic v2 candidate drifted")
    if s["filter_1"] != "p_parent_integrity_augmented_strictly_greater_than_p_severity_only":
        raise RuntimeError("economic v2 edge filter drifted")
    if s["filter_2"] != "structural_expected_net_return_strictly_greater_than_zero":
        raise RuntimeError("economic v2 expectancy filter drifted")
    if float(s["expectancy_threshold"]) != 0.0 or s["expectancy_threshold_search"] is not False:
    ...
    if p["VALIDATION"]["gate_change_from_v1"] is not False:
        raise RuntimeError("economic v2 validation gates drifted from v1")
    return p
...
def read_source(path: Path) -> pd.DataFrame:
    ...
    if (frame["trading_day"] >= "2026-01-01").any():
        raise RuntimeError("economic v2 DEV/VALIDATION runner read blackbox")
...
def trade_events(
    ...
    for lower in lower_waves:
        ...
        recovery_gross = float(parent_sign * (recovery / entry_price - 1.0))
        failure_gross = float(parent_sign * (failure / entry_price - 1.0))
        if not (recovery_gross > 0.0 and failure_gross < 0.0):
            continue
        ...
        gross = float(parent_sign * (exit_price / entry_price - 1.0))
...
def add_filters(events: pd.DataFrame, bundle: dict) -> pd.DataFrame:
...
def validation_pair(events: pd.DataFrame, dev_bundle: dict, pair_id: str, protocol: dict) -> dict:
    scored = add_filters(events, dev_bundle)
...
def final_freeze(pair_events: dict[str, pd.DataFrame], protocol: dict) -> dict:
    payload = {
        "schema_id": "factorlab_rmr_R1_economic_translation_v2_parameter_freeze@1.0",
        ...
        "strategy_candidate_id": "R1_STRUCTURAL_EXPECTANCY_POSITIVE_NEXT_BAR_10BP",
        ...
        "filter_1": "p_parent_integrity_augmented_strictly_greater_than_p_severity_only",
        "filter_2": "structural_expected_net_return_strictly_greater_than_zero",
    ...
    for pair_id, events in pair_events.items():
        bundle = econ1.fit_probability_bundle(events, VAL_END)
...
def main() -> int:
    ...
    for pair_id, lower, parent in PAIRINGS:
        ...
        dev_bundle = econ1.fit_probability_bundle(events, DEV_END)
    ...
    receipt = {
        "schema_id": "factorlab_rmr_R1_economic_translation_v2_validation_receipt@1.0",
        "session_date": "2026-09-08",
        "research_identity": "rmr_R1_parent_integrity_economic_translation_v2",
    ...
    if not passed:
        print("R1_ECONOMIC_TRANSLATION_V2_VALIDATION_FAIL")
        return 2
    freeze = final_freeze(pair_events, protocol)
    dump_json(args.freeze.resolve(), freeze)
    print("R1_ECONOMIC_TRANSLATION_V2_VALIDATION_PASS_FINAL_REFIT_FROZEN")
    return 0
