#!/usr/bin/env python3
"""Reusable low-bandwidth blackbox certifier for R1 economic translation v1."""
...
import run_rmr_R1_economic_translation_v1 as econ
...
DEFAULT_RECEIPT = ROOT / "docs/research/local_rmr_R1_economic_translation_v1_blackbox_receipt.json"
...
def validate_freeze(path: Path) -> tuple[dict, dict]:
    ...
    if freeze["research_identity"] != "rmr_R1_parent_integrity_economic_translation_v1":
        raise RuntimeError("economic blackbox identity drifted")
    if freeze["strategy_candidate_id"] != "R1_EDGE_POSITIVE_NEXT_BAR_10BP":
        raise RuntimeError("economic strategy candidate drifted")
    if freeze["blackbox_used_in_fit"] is not False:
        raise RuntimeError("economic strategy fit used blackbox")
    if freeze["round_trip_cost_bps"] != 10.0:
        raise RuntimeError("economic cost drifted")
    expected = freeze["parameter_bundle_sha256"]
    check = dict(freeze)
    check.pop("parameter_bundle_sha256")
    if json_digest(check) != expected:
        raise RuntimeError("economic final bundle digest mismatch")
    return protocol, freeze


def read_frame(path: Path, expected_sha: str, start: str, end: str) -> pd.DataFrame:
    if sha256(path) != expected_sha:
        raise RuntimeError("economic blackbox source identity mismatch")
    ...
    if frame.empty:
        raise RuntimeError("economic blackbox source empty")
    vals = frame["close"].to_numpy(float)
    if not np.isfinite(vals).all() or (vals <= 0).any():
        raise RuntimeError("economic blackbox invalid close values")
...
def combine(hist: pd.DataFrame, blackbox: pd.DataFrame) -> pd.DataFrame:
    if str(hist["trading_day"].max()) != "2025-12-31":
        raise RuntimeError("economic historical context end drifted")
    if str(blackbox["trading_day"].min()) != BLACKBOX_START or str(blackbox["trading_day"].max()) != BLACKBOX_END:
        raise RuntimeError("economic blackbox window incomplete")
    ...
    if out.duplicated(["symbol", "trading_day", "timestamp"]).any():
        raise RuntimeError("economic combined source duplicate minute")
    return out.reset_index(drop=True)


def add_frozen_edge(events: pd.DataFrame, frozen_pair: dict) -> pd.DataFrame:
    out = frozen_eval.add_frozen_integrity(events, frozen_pair["parent_feature_scaler"])
    p_base = frozen_eval.frozen_predict(frozen_pair["severity_baseline_model"], out[["severity"]].to_numpy(float))
    p_cand = frozen_eval.frozen_predict(
        frozen_pair["selected_candidate_model"], out[["severity", "parent_integrity"]].to_numpy(float)
    )
    out["selected_trade"] = p_cand > p_base
    return out
...
def pairing_gate(events: pd.DataFrame, frozen_pair: dict, minimum: int) -> tuple[bool, bool]:
    ...
    scored = add_frozen_edge(bb, frozen_pair)
...
def main() -> int:
    ...
    for pair_id, lower, parent in PAIRINGS:
        events = econ.trade_events(prices, days, waves[lower], waves[parent], vol_ref)
    ...
    query_id = hashlib.sha256(f"R1ECON|{candidate_fp}|{protocol_fp}|{BLACKBOX_SHA}".encode()).hexdigest()[:20]
    receipt = {
