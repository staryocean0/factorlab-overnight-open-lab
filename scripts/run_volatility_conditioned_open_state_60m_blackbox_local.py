#!/usr/bin/env python3
"""Low-bandwidth reusable BLACKBOX controller for frozen OFP-C2 60m.
...
IDENTITY = "overnight_volatility_conditioned_open_state_60m_v1"
SYMBOL = "000852.SH"
...
def standardized_interaction_coefficient(frame: pd.DataFrame) -> float:
    ...
    design = np.column_stack(
        [np.ones(len(x)), x[[*BASELINE_CONTROLS, "vol_gap_interaction"]].to_numpy(float)]
    )
...
def partial_corr(frame: pd.DataFrame) -> float:
    ...
    interaction = frame["vol_gap_interaction"].to_numpy(float)
...
def load_target(minute_bars: Path) -> pd.DataFrame:
    ...
    bars = bars.loc[bars["clock"].isin(["09:35", "10:35"])].copy()
    bars["close"] = pd.to_numeric(bars["close"], errors="coerce")
    wide = bars.pivot_table(index="trading_day", columns="clock", values="close", aggfunc="last")
    if "09:35" not in wide.columns or "10:35" not in wide.columns:
        raise RuntimeError("frozen 60m BLACKBOX clocks unavailable")
    out = pd.DataFrame(
        {
            "trading_day": wide.index.astype(str),
            "target": (wide["10:35"] / wide["09:35"] - 1.0).to_numpy(),
        }
    )
...
def main() -> int:
    ...
    if protocol.get("research_identity") != IDENTITY:
        raise RuntimeError("wrong C2 60m BLACKBOX protocol")
    ...
    panel["log_rvol20"] = np.log(panel["rvol20"])
    panel["vol_gap_interaction"] = panel["observed_gap_rvol"] * panel["log_rvol20"]
    ...
    receipt = {
        "schema_id": "overnight_volatility_conditioned_open_state_60m_blackbox_receipt@1.0",
        "query_id": query_id,
        "research_identity": IDENTITY,
        "comparator": "same_controls_without_vol_gap_interaction",
        "decision": decision,
