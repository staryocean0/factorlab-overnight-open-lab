...
计划身份：`overnight_extreme_open_conditional_transition_program_v1`；状态：`P1_COMPLETE_6_SURVIVORS_P2_COMPLETE_NO_SURVIVOR_P3_FREEZE_REQUIRED`。当前 `active_research`：null（没有获准执行的 outcome-bearing identity）；结果型执行权限为 `False`。
