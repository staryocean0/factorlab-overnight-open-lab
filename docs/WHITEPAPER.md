...
计划身份：`overnight_extreme_open_conditional_transition_program_v1`；状态：`P5_REUSABLE_VALIDATION_FROZEN_QUERY_NOT_YET_AUTHORIZED`。当前 `active_research`：null（没有获准执行的 outcome-bearing identity）；结果型执行权限为 `False`。
