#!/usr/bin/env python3
"""CT-AUDIT-A: sealed 2011-2012 validation of frozen CSI300/CSI500 gap-fill hazards.

No T1/T2 model or scaler is refit. Audit B, the 2014 supporting crosscheck and
CSI1000 post-2026-08-21 outcomes are never loaded. Only aggregate receipts are
written.
"""
...
def read_symbol(source: Path, symbol: str) -> pd.DataFrame:
    ...
    if frame.empty:
        raise RuntimeError(f"no Audit-A rows for {symbol}")
    frame["trading_day"] = frame["trading_day"].astype(str)
    if frame["trading_day"].max() > AUDIT_END or (frame["trading_day"] >= "2013-01-01").any():
...
def build_audit_frame(source: Path, name: str, symbol: str) -> tuple[pd.DataFrame, dict]:
    ...
    for _, row in audit_daily.iterrows():
        ...
        rows.append(result)
    frame = pd.DataFrame(rows).sort_values("trading_day", kind="mergesort").reset_index(drop=True)
    if frame.empty or frame["trading_day"].min() < AUDIT_START or frame["trading_day"].max() > AUDIT_END:
        raise RuntimeError(f"{name} invalid Audit-A target inventory")
...
def evaluate_t2(frame: pd.DataFrame, sign: str, bundle: dict) -> dict:
    ...
    if part.empty:
        return {"counts": {"all": 0, "gt10bp": 0, "gt30bp": 0}, "model": {}, "benchmark": {}, "sample_sufficient": False, "gates": None, "gate_pass_count": None, "audit_a_passed": False, "monotonicity_violations": 0, "annual_all_gap": {}}
    ...
    if sufficient:
        ...
        out["audit_a_passed"] = bool(all(out["gates"].values()))
    else:
        out["gates"], out["gate_pass_count"], out["audit_a_passed"] = None, None, False
    annual = {}
    years = part["year"].to_numpy(int)
    for y in [2011, 2012]:
        mask = years == y
        yp = part.loc[mask].copy().reset_index(drop=True)
        if len(yp):
            annual[str(y)] = {"model": safe_eval(yp, tuple(p[mask] for p in mp), False), "benchmark": safe_eval(yp, tuple(p[mask] for p in bp), False)}
    out["annual_all_gap"] = annual
    return out
...
def main() -> int:
    ...
    if protocol["stage"] != "CT_AUDIT_A_preregistered_unopened" or protocol["audit_a_opened"] or protocol["audit_b_opened"]:
        raise RuntimeError("Audit-A protocol state drifted")
    ...
    for name, symbol in {"CSI300": "000300.SH", "CSI500": "000905.SH"}.items():
        ...
        for sign in ["high", "low"]:
            ...
            head_pass.append(bool(t2[sign]["audit_a_passed"]))
    ...
    eligible = bool(all_sufficient and all_pass)
    decision = "CT_AUDIT_A_all_four_heads_pass_cloud_review_required_before_Audit_B" if eligible else ("CT_AUDIT_A_evidence_insufficient_do_not_open_Audit_B" if not all_sufficient else "CT_AUDIT_A_not_all_four_heads_pass_do_not_open_Audit_B")
    receipt = {
        "schema_id": "overnight_open_gap_fill_cross_index_audit_a_receipt@1.0", "session_date": "2026-09-07", "research_identity": "gap_fill_cross_index_transport_v1", "stage": "CT_AUDIT_A_completed_local_pending_cloud_review",
        "protocol": str(PROTOCOL.relative_to(ROOT)), "source_dataset_version": ADMITTED_DATASET_VERSION, "source_dataset_sha256_from_HE00": ADMITTED_DATASET_SHA256,
        "external_parameter_artifact_sha256": EXPECTED_EXTERNAL_PARAMETER_FILE_SHA256, "CSI300_parameter_bundle_sha256": EXPECTED_CSI300_BUNDLE_SHA256, "CSI500_parameter_bundle_sha256": EXPECTED_CSI500_BUNDLE_SHA256,
        "CSI1000_architecture_sha256": EXPECTED_CSI1000_ARCH_SHA256, "CSI1000_parameter_bundle_sha256": EXPECTED_CSI1000_BUNDLE_SHA256,
        "source_audits": audits, "indices": results, "all_four_T2_heads_sample_sufficient": all_sufficient, "all_four_T2_heads_pass_all_six_gates": all_pass,
        "audit_b_eligibility_under_frozen_rule": eligible, "decision": decision,
        "T1_refit_performed": False, "T2_refit_performed": False, "scaler_refit_performed": False, "feature_search_performed": False, "model_class_search_performed": False,
        "hyperparameter_search_performed": False, "threshold_search_performed": False, "probability_calibration_performed": False, "audit_b_opened": False, "supporting_crosscheck_opened": False,
        "csi1000_post_2026_08_21_outcomes_opened": False, "fresh_oos": False, "production_authority": False,
    }
    usage = {
        "schema_id": "overnight_open_gap_fill_cross_index_audit_a_data_usage@1.0", "session_date": "2026-09-07", "research_identity": "gap_fill_cross_index_transport_v1",
        "opened": {"CSI300_Audit_A_2011-01-01_to_2012-12-31": True, "CSI500_Audit_A_2011-01-01_to_2012-12-31": True}, "feature_history_only": {"2010-10-01_to_2010-12-31": True},
        "sealed": {"Audit_B_2013-01-01_to_2014-10-16": True, "supporting_crosscheck_2014-10-17_to_2014-12-31": True, "CSI1000_2026Q4_true_fresh": True},
        "aggregate_outputs_only": True, "raw_rows_written_to_repo": False, "production_authority": False,
    }
    dump_json(OUT, receipt); dump_json(USAGE, usage)
    print("GAP_FILL_CROSS_INDEX_AUDIT_A_RESULT", json.dumps({"decision": decision, "audit_b_eligibility_under_frozen_rule": eligible, "all_four_sample_sufficient": all_sufficient, "all_four_pass": all_pass, "audit_b_opened": False}, sort_keys=True))
    return 0
